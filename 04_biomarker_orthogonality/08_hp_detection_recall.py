...
n_hp = int((df['hypo_k3_n14'] == HP_CODE).sum())
hp_prev = n_hp / len(df)
...
rec(f'>={FAST_MIN} h self-reported fasted: n={len(df_fast):,} '
    ...
    f'{int((df_fast["hypo_k3_n14"] == HP_CODE).sum())}')
...
if lab_vals.size:
    ...
    # Every lab arm fell below the leaf rule, so none is plotted and there is no range to
    # quote. The per-arm recalls above are still reported, flagged as underpowered.
    rec('  lab arms across models     no arm met the leaf rule; see the per-arm rows above')
...
n_hp_f = int((fast_cc['hypo_k3_n14'] == HP_CODE).sum())
