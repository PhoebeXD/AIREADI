...
PRUNED = f'{BASE}/canonical_n14_rerun'
OUT    = f'{PRUNED}/subtype_classification'
...
_META = json.load(open(f'{PRUNED}/models/meta.json'))
...
# AI-READI has no fasting flag (self-report only, cohort median 3 h since eating). Figure 5b
# was rebuilt in July to avoid exactly this; para 84's analysis was not, so the paper was
# applying two standards to one question. Now MATCHES FIGURE 5b's three escalating arms:
# best single clinical test -> broad fasting-clean panel -> complete panel on valid specimens.
...
assert DF['hypo_k3_pruned'].isna().sum() == 0
NAME3 = {0: 'Spiker', 1: 'Stable', 2: 'HP'}
DF['grp'] = DF['hypo_k3_pruned'].map(NAME3)
...
print(f'fasted (>={FAST_MIN} h self-report): n={int(FASTED.sum())} '
      ...
      f'{int((FASTED & (DF["hypo_k3_pruned"] == 2)).sum())}')
