...
N14_TABLE = os.path.join(BASE, "canonical_n14_rerun/processed/analysis_table_n1306_n14.csv")
...
# Values reported in the manuscript for the AI-READI untreated cohort: the share of
# glucose_sd and glucose_cv variance explained by HbA1c (squared Pearson). They are here
# only so a run against that cohort can say whether it reproduced them, and are expected
# to differ on any other dataset -- a mismatch is reported, never enforced.
PUBLISHED_REFERENCE = {"sd_r2": 0.209, "cv_r2": 0.058}
REFERENCE_TOL = 0.01
...
# =====================================================================
pt = pd.read_csv(N14_TABLE)
vc = pt.hypo_k3_n14.value_counts().to_dict()
print(f"[labels] N14_TABLE n={len(pt)}  Spiker:{vc.get(0)} Stable:{vc.get(1)} HP:{vc.get(2)}")
assert len(vc) == 3, 'expected three pattern labels'

# =====================================================================
# PANEL a — R² cliff:  R² = Pearson(HbA1c, feature)**2  (squared Pearson)
# =====================================================================
# An earlier version of this panel plotted the 10 frozen pruned-10 clustering features,
...
print(f"   glucose_sd (derived)  R2={sd_r2:.4f}")
print(f"   glucose_cv (derived)  R2={cv_r2:.4f}")

# Reproducibility check, reported and not enforced. On the AI-READI untreated cohort both
# should land within REFERENCE_TOL of PUBLISHED_REFERENCE; on any other dataset -- the
# synthetic demo included -- they will not, and that is not an error.
_checks = {"sd_r2": sd_r2, "cv_r2": cv_r2}
_hits = {k: abs(v - PUBLISHED_REFERENCE[k]) <= REFERENCE_TOL for k, v in _checks.items()}
print(f"\n[panel a] vs published reference (tol {REFERENCE_TOL}):")
for k, v in _checks.items():
    ref = PUBLISHED_REFERENCE[k]
    print(f"   {k:<8s} computed={v:.4f}  published={ref:.4f}  "
          f"delta={v - ref:+.4f}  {'within' if _hits[k] else 'OUTSIDE'} tolerance")
if all(_hits.values()):
    print("   -> reproduces the published values")
else:
    print("   -> does NOT reproduce the published values; expected unless this run is "
          "against the AI-READI untreated cohort")
...
# Fasting-dependent markers are excluded from panel b. The cohort carries no fasting flag,
# only self-reported hours since last ate, so on the full cohort these values are
# postprandially contaminated. Triglycerides is the most fasting-sensitive lipid, and TG/HDL
# and calculated LDL are derived from it, so all three inherit the problem.
#
# Restricting them to the >=12 h fasted subset was tried and rejected. That subset is small
# and self-selected, and because best_abs() takes the MAXIMUM |rho| over the hypo metrics,
# the winning metric would be chosen and estimated on a fraction of the cohort, where a
# maximum over many candidates is upward biased. The exclusion is therefore a property of
# the estimator on a small subset, not a claim that no association exists there; the fasted
# lipid analysis is reported separately. See Methods.
#
# Excluded from panel b, in full: homa_ir_corrected, fasting_glucose, fasting_insulin,
# hepatic_ir, c_peptide, triglycerides, tg_hdl, ldl.
...
# =====================================================================
# PANEL c — matched-mean HbA1c (Spiker vs Stable only)
# =====================================================================
d = pt.dropna(subset=["hba1c","mean_glucose"]).copy()
d["pattern"] = d.hypo_k3_n14.map(NAME)
...
SD_ = {g: pt.loc[pt.hypo_k3_n14.map(NAME) == g, "hba1c"].dropna().values for g in ORDER_D}
NLAB_D = {g: int((pt.hypo_k3_n14.map(NAME) == g).sum()) for g in ORDER_D}
