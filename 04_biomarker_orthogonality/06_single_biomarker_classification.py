...
TABLE  = f'{PRUNE}/processed/analysis_table_n1306_n14.csv'
...
y_all = df['hypo_k3_pruned'].values
...
def evaluate(cols, label, classes, fasted=True):
    ...
    base = base[base['hypo_k3_pruned'].isin(classes)]
    sub = base.dropna(subset=cols)
    X = sub[cols].values.astype(float); y = sub['hypo_k3_pruned'].values
...
def build(K):
    ...
    df_f = df[(df['fast_h'] >= FAST_MIN) & (df['hypo_k3_pruned'].isin(CLASSES))].copy()
    grp_str = ', '.join(f'{SHORT[g]} {int((df_f.hypo_k3_pruned == g).sum())}' for g in CLASSES)
    ...
    n_irpanel = len(subIR)
    prev = subIR.groupby('hypo_k3_pruned')['HighIR'].mean() * 100
    grp_n = subIR.groupby('hypo_k3_pruned')['HighIR'].size()
    grp_hi = subIR.groupby('hypo_k3_pruned')['HighIR'].sum()
    ss = subIR[subIR['hypo_k3_pruned'].isin([0, 1])]          # Spiker+Stable enrichment contrast
    pooled_hi = ss['HighIR'].mean() * 100
    ct = pd.crosstab(ss['hypo_k3_pruned'], ss['HighIR']).reindex(index=[0, 1], columns=[0, 1]).fillna(0)
