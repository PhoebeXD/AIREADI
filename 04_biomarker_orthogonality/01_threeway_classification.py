...
DF['lab'] = DF['hypo_k3_n14']

CGM14 = ['mean_glucose', 'glucose_sd', 'glucose_cv', 'mage', 'pct_above_140',
...
assert CGM14 == json.load(open(f'{BR}/models/meta.json'))['features'], \
...
# complete-case rows for the biomarker panel (CGM14 has zero missing, so the
...
h("Same 3-class target throughout: n14 (canonical-15 minus `n_lows_54`, tz-corrected "
  ...
  f"(`hypo_k3_n14`, Spiker {int((DF.lab==0).sum())} / Stable {int((DF.lab==1).sum())} / "
...
h()
h(f"1. **CGM-only** ({len(CGM14)} n14 clustering features): "
  f"`{', '.join(CGM14)}`")
h(f"2. **Biomarker-only** (routine fasting panel, {len(PANEL)} markers, all present in "
  f"the table): `{', '.join(PANEL)}`")
h(f"3. **Combined** ({len(CGM14) + len(PANEL)} features): CGM {len(CGM14)} + biomarker panel, "
...
h(f"- **CGM-only full-n: {n_full}** (Spiker {int((DF.lab==0).sum())} / Stable {int((DF.lab==1).sum())} / HP {int((DF.lab==2).sum())}) — reported "
  "separately so the headline is NOT silently reduced by biomarker missingness.")
h()
...
full_cgm = {}
for model in MODELS:
    X = DF[CGM14].values.astype(float)
...
for k, model in enumerate(MODELS):
    ...
    # Clip at zero: when balanced accuracy saturates, the bootstrap percentile CI can sit a
    # hair above or below the point estimate, and matplotlib rejects a negative error bar.
    # Presentation only -- the plotted CI bounds are unchanged wherever they bracket the mean.
    los = [max(0.0, results[(s, model)]['bacc_mean'] - results[(s, model)]['ci_lo'])
           for s in sets]
    his = [max(0.0, results[(s, model)]['ci_hi'] - results[(s, model)]['bacc_mean'])
           for s in sets]
    pos = xb + (k - 0.5) * width
...
fig.suptitle('Three-way individual classification of n14 K=3 membership '
