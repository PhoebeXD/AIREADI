...
fix = pd.read_csv(f'{BASE}/pruned_n10_tzfix/processed/day_night_diff_local_all.csv')
...
# ---------------------------------------------------------------- outputs
# battery-compatible column name so downstream scripts can be repointed unchanged
pd.DataFrame({'person_id': at.person_id, 'hypo_k3_n14': lab, 'hypo_k3_pruned': lab,
              'hypo_k3_canonical': ref['canonical-15 (UTC)'], 'raw_cluster': raw}
             ).to_csv(f'{NEW}/labels/hypo_clustering_n14_n1306.csv', index=False)
out = at.copy(); out['hypo_k3_pruned'] = lab
