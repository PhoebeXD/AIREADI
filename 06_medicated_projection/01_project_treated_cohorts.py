...
out  processed/medicated_features_pruned.csv, logs/medicated_projection_n14.csv,
...
PRUNED=['mean_glucose','glucose_sd','glucose_cv','mage','pct_above_140','pct_above_180',
...
# UAB=cst(-6), UW/UCSD=pst(-8), non-DST — matches scripts/63_circadian_signature.py.
...
# ---- canonical extractor (verbatim from script 15) ----
...
m=canon[['person_id']+PRUNED].merge(unmed_feat[['person_id']+PRUNED], on='person_id', suffixes=('_canon','_recomp'))
...
med_feat.to_csv(f'{NEW}/processed/medicated_features_pruned.csv', index=False)

# load pruned model
# pickles in pruned_n10_rerun/models/ which were fit on the UTC-buggy feature.
...
def project(feat_df):
    X=feat_df[PRUNED].fillna(feat_df[PRUNED].median()).values
...
print('\n=== PRUNED K=3 prevalence across medication status ===')
...
print(f'Saved: {NEW}/logs/medicated_projection_pruned.csv, processed/medicated_features_pruned.csv')
