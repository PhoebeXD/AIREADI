...
# ---------------------------------------------------------------- load / compute
lab = pd.read_csv(LAB, usecols=["person_id", "hypo_k3_pruned"]).rename(columns={"hypo_k3_pruned": "k3"})
...
with open(LOGMD, "w") as f:
    ...
    f.write("```\n" + hdr.strip() + "\n```\n\n")
    f.write(f"Cohort (pruned labels): Spiker {ns['Spiker']} / Stable {ns['Stable']} "
...
fig.suptitle(f"Circadian baseline vs excursion, Spiker vs Stable (pruned labels) — "
