...
AT     = f'{BR}/processed/analysis_table_n1306_n14.csv'
CGM    = f'{BASE}/cgm_all_readings_clean_final.csv'
LOGDIR = f'{BR}/logs'
UTC_OFFSET = {'pst': -8, 'cst': -6}
NAME = {0: 'Spiker', 1: 'Stable', 2: 'HP'}

lab = pd.read_csv(AT, usecols=['person_id', 'hypo_k3_n14', 'timezone'])
lab['timezone'] = lab['timezone'].fillna('pst').str.lower()
vc = lab['hypo_k3_n14'].value_counts().to_dict()
assert len(vc) == 3, 'expected three pattern labels'
g3 = dict(zip(lab.person_id, lab.hypo_k3_n14))
...
md.append('\n> The count split is partly mechanical: the waking window spans 18 of 24 h, so '
          '**75% waking / 25% overnight is the uniform null** against which it should be read. '
          'The wear-normalized rate below removes the window-size confound.\n')
