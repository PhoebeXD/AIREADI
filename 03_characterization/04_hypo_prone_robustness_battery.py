...
HP_PRUNED = set(LAB.loc[LAB.hypo_k3_pruned == 2, 'person_id'].astype(int))
HP_CANON = set(LAB.loc[LAB.hypo_k3_canonical == 2, 'person_id'].astype(int))
ALLPIDS = LAB['person_id'].astype(int).tolist()
print(f'HP pruned {len(HP_PRUNED)} | canonical {len(HP_CANON)}')
...
F = FEAT.merge(LAB[['person_id', 'hypo_k3_pruned', 'hypo_k3_canonical']], on='person_id')
...
w()
w('Seed 42 throughout. HP-core = the `hypo_k3_pruned`==2 members ('
  'kept for cross-check). Pre-registered downgrade thresholds decided before results: '
...
w('Rank persons by a single hypo-burden axis, sweep the cutoff, and compute Jaccard overlap '
  'with the 36 cluster-HP. High overlap => HP does **not** depend on the soft cluster boundary; '
...
for name, ser in axes.items():
    ...
    for cut in np.unique(ser.dropna().values):
        ...
        j = jaccard(sel, HP_PRUNED)
        if j > best[0]:
            inter = len(sel & HP_PRUNED)
            prec = inter/len(sel) if sel else 0
            rec = inter/len(HP_PRUNED)
...
n_runs = len(runs)
# per-person HP frequency among the 36 pruned-HP
freq = {p: np.mean([p in hp for _, hp in runs]) for p in HP_PRUNED}
freqser = pd.Series(freq).sort_values()
core80 = [p for p, f in freq.items() if f >= 0.80]
recurrence = len(core80) / len(HP_PRUNED)
mean_freq = np.mean(list(freq.values()))
# HP<->Spiker crossing: among pruned-HP, fraction of (person,run) assigned to a NON-HP cluster
...
# recompute spiker membership per run is heavy; approximate crossing via: of pruned-HP not in
...
w()
w(f'- **Stable HP-core (≥80% of runs): {len(core80)} / {len(HP_PRUNED)} members → recurrence '
...
w(f'- **HP↔Spiker crossing rate** (pruned-HP assigned to the Spiker cluster, 20 seed runs): '
...
w()
rc = RAW[RAW.person_id.isin(HP_PRUNED)].copy()
# classify each RUN
...
w()
# use lows.parquet (sustained events) restricted to pruned-HP with adequate wear in both halves
hpqc = QC[(QC.group == 'HP')]
hp_incl = set(hpqc.loc[hpqc.included, 'person_id'].astype(int))
qci = QC[(QC.group == 'HP') & QC.included].set_index('person_id')
lh = LOWS[LOWS.person_id.isin(HP_PRUNED)]
...
w(f'Among the {len(hp_incl)} HP-core members with adequate wear in **both** halves '
  f'(2 of 36 excluded by wear-QC: {sorted(set(int(p) for p in HP_PRUNED)-hp_incl)}); '
...
if not any_trip:
    ...
    w('Tripped thresholds: ' + '; '.join(f'**{t}** ({d}, value {v})' for t, d, v in TRIPS) +
      '. Report HP as tentative; let orthogonality carry the paper.')
w()
w(f'**Hard limit no test removes: n=36.** Independent-cohort replication (a second CGM cohort with '
