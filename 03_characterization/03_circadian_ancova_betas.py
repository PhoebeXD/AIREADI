...
OUT_MD    = os.path.join(BASE, "logs/circadian_signature_n14.md")

UTC_OFFSET = {"pst": -8, "cst": -6}
NAME = {0: "Spiker", 1: "Stable", 2: "HP"}

# ---------------------------------------------------------------- labels + tz
lab = pd.read_csv(LAB, usecols=["person_id", "hypo_k3_n14"])
...
vc = meta.hypo_k3_n14.value_counts().to_dict()
print(f"[load] n14 cohort n={len(meta)}  groups={{Spiker:{vc.get(0)}, Stable:{vc.get(1)}, HP:{vc.get(2)}}}")
...
feat["group"] = feat.hypo_k3_n14.map(NAME)
feat.to_csv(OUT_FEAT, index=False)
print(f"[save] {OUT_FEAT}  ({len(feat)} rows)")

# ---------------------------------------------------------------- ANCOVA: beyond burden
two = feat[feat.hypo_k3_n14.isin([0, 1])].copy()
two["is_spiker"] = (two.hypo_k3_n14 == 0).astype(int)
SP, ST, HP = (feat[feat.hypo_k3_n14 == k] for k in (0, 1, 2))
...
with open(OUT_MD, "w") as f:
    f.write(f"# Circadian ANCOVA betas ({'/'.join(str(int(vc[i])) for i in sorted(vc))})\n\n")
    f.write("AI-READI v3.0.0; Spiker vs Stable, adjusted for 24h mean glucose; HP excluded "
            f"(n={len(HP)} descriptive).\n\n")
    ...
    f.write("\nPanel-c headline (n14): nocturnal beta_adj = "
