...
NEW  = f'{BASE}/local_time_refit'
# Pre-timezone-fix SNAPSHOT of the pruned-10 table. It must be the snapshot, not a working
# copy: this script's provenance check (below) asserts the stored day_night_diff is still
# the UTC variant, and a working copy that has had the fix applied to it will not pass.
OLD_TABLE = f'{BASE}/archive_tzfix_backup_20260801/analysis_table_n1306_pruned.csv'
...
UTC_OFFSET = {'pst': -8, 'cst': -6}          # non-DST
...
def dnd_local(path, tag):
    ...
    cgm['off'] = cgm['off'].fillna(-8).astype(int)       # pst default
...
# The recomputed day/night means are written only once the provenance check above has passed.
# `02_clustering/01` consumes this file, so writing it before validating would hand a
# downstream step an input from a run that then aborted.
alld.to_csv(f'{NEW}/processed/day_night_diff_local_all.csv', index=False)
emit(f'\nwrote processed/day_night_diff_local_all.csv  (n={len(alld):,})')
...
with open(f'{NEW}/logs/refit_tzfix_log.md', 'w') as f:
    ...
    f.write('```\n' + '\n'.join(log) + '\n```\n')
emit('\nWROTE labels/, processed/, models/, logs/ under local_time_refit/')
